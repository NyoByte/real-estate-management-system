    ...
    let instance = RealEstateManagement.deployed().then(instance => {
        ...
        instance.getPropertyHashById(0).then(function (hash) {
            ...
            for (let i = 0; i < iterations; i++) {
                ...
                let start = new Date().getTime()
                instance.createSell(20, hash, accounts[index], 1, { from: accounts[0] }).then(function (result) {
                    numTransactions++
                    let end = new Date().getTime()
                    let time = end - start
                    ...
                    if (result.receipt.blockNumber != currentBlock  || i == iterations - 1) {
                        console.log("Block number: " + currentBlock)
                        console.log("Transactions: " + numTransactions)
                        console.log("Throughput: " + numTransactions / ((new Date().getTime() - blockStart) / 1000) + " transactions/s")
                        currentBlock = result.receipt.blockNumber
