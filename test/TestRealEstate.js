...
contract('RealEstateManagement', function (accounts) {
    ...
    it("Test User creation", async () => {
        ...
        var receipt = await rem.createNewUser("User1", "xxx", "Lima", "La Molina", walletUser1_pow, "12345678", { from: accounts[1] });
        ...
        await rem.createNewUser("User2", "xxx", "Lima", "La Molina", walletUser2_pow, "74125863", { from: accounts[1] });
        await rem.createNewUser("User3", "xxx", "Lima", "Ate", walletUser3_pow, "12398546", { from: accounts[1] });
        await rem.createNewUser("User4", "xxx", "Lima", "Jesus María", walletUser4_pow, "32458716", { from: accounts[1] });
        const userAddress = await rem.getUserAddressById(0)
        //assert.equal(initial - final, gasUsed, "Consumo diferente xd");
        assert.equal(userAddress, walletUser1_pow, "User not created correctly");
        ...
        // Obtain gas used from the receipt
        receipt = await rem.createnNewProperty("Lima", "San Luis", "Av. Siempre viva 123", 120, [walletUser1_pow, walletUser2_pow], [50,50], { from: accounts[1] });
        ...
        initial = await web3.eth.getBalance(walletUser1_pow);
        console.log(`Gas Initial: ${initial.toString()}`);

        start = new Date().getTime();
        // Obtain gas used from the receipt
        receipt = await rem.createSell(2,propertyHash,walletUser3_pow,50, {from: accounts[2]});
        end = new Date().getTime();
        console.log(`Transaction time: ${end - start}`);
        gasUsed = receipt.receipt.gasUsed;
        ...
        // Final balance
        final = await web3.eth.getBalance(walletUser1_pow);
        console.log(`Gas Final: ${final.toString()}`);
        ...
        initial = await web3.eth.getBalance(walletUser3_pow);
        console.log(`Gas Initial: ${initial.toString()}`);
        ...
        // Final balance
        final = await web3.eth.getBalance(walletUser3_pow);
        console.log(`Gas Final: ${final.toString()}`);
